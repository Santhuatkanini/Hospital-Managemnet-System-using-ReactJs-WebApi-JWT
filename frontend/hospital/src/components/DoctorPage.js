  ...
  const fetchAppointments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        // Filter appointments by doctorId and status
        const doctorAppointments = appointmentsData.filter(
          (appointment) =>
            String(appointment.doctorId) === loggedInDoctorId &&
  ...
  const fetchViewAppointments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const doctorAppointments = appointmentsData.filter(
          (appointment) => String(appointment.doctorId) === loggedInDoctorId
        );
  ...
  const fetchCancelAppointments = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const cancelAppointments = appointmentsData.filter(
          (appointment) =>
            String(appointment.doctorId) === loggedInDoctorId &&
            (appointment.status === "Appointment Pending" ||
        ...
        setAppointments(cancelAppointments);
      } else {
        console.error("Failed to fetch appointments");
      }
    } catch (error) {
